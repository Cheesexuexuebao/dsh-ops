/**
 * Build the client face as a CJS bundle with react external; zod is inlined
 * so the browser module table needs no extra row. Then wrap the output into
...
await build({
  ...
  bundle: true,
  sourcemap: false,
  external: ["react", "react/*", "react-dom", "react-dom/*", "react-dom/client", "@deepseek-ai/*"],
  logLevel: "info",
});
