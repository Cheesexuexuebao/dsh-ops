/**
 * Durable SSH resource management for Settings → Plugins (dsh-ssh-connection).
 * Server coordinates live in the host storage domain; secrets only cross the
 * web boundary through DSH's credentials.set/unset API.
 *
 * Connect builds ctx.ssh transport only. If dsh-ssh-ops is loaded it may open
 * a terminal after hearing `dsh-ssh-connection:connected`.
 */
import * as React from "react";

const { useEffect, useRef, useState } = React;
const CONNECTED_EVENT = "dsh-ssh-connection:connected";
const DISCONNECTED_EVENT = "dsh-ssh-connection:disconnected";
  ...
  const renderProfiles = (items) => items.length === 0 ? null : <div style={styles.list}>{items.map((profile) => (
    <div key={profile.profileId} style={styles.card}>
      ...
      <div style={styles.cardActions}>
        {profile.connected ? (
          <button type="button" disabled={disconnecting === profile.profileId || !profile.connectionId} onClick={() => disconnect(profile)} style={styles.primary}>{disconnecting === profile.profileId ? "关闭中…" : "关闭连接"}</button>
        ) : (
          <button type="button" disabled={connecting === profile.profileId || !profile.credentialConfigured} onClick={() => connect(profile)} style={styles.primary}>{connecting === profile.profileId ? "连接中…" : "连接"}</button>
        )}
