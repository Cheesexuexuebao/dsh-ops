...
describe('connection node half', () => {
  ...
  it('admits privileged methods for a declared trusted authority (ops LAN)', async () => {
    const { routes, dispose } = await mounted({ trustedHosts: ['harness.example'] })
    // Privileged methods share the Host fence with ordinary /api traffic: a
    // declared trusted authority reaches them (carrier 404 from the empty
    // proxy proves the fence passed). Undeclared hosts still 403 earlier.
    for (const method of [
    ...
    expect(foreign.state.body).toBe('forbidden')
    await dispose()
  })
...
describe('connection node half over a real HTTP server', () => {
  ...
  it('admits configuration methods for a declared LAN authority over real HTTP', async () => {
    // Fence input is a real IncomingMessage Host header (LAN browser shape).
    const { routes, dispose } = await mounted({ trustedHosts: ['harness.example'] })
